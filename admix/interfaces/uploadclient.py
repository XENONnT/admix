...
#                    domain = 'lan'
    ...
    def _register_file(self, file, registered_dataset_dids,helper):
        """
        ...
        if dataset_did_str and dataset_did_str not in registered_dataset_dids:
            ...
            try:
                ...
                helper.global_dictionary['logger'].Info('     3.1')
...
#        except DataIdentifierNotFound:
    ...
    def _collect_and_validate_file_info(self, items):
        ...
        for item in items:
            ...
            pfn = item.get('pfn')
            if not path:
                print('Skipping source entry because the key "path" is missing')
                continue
            if not item.get('rse'):
                print('Skipping file %s because no rse was given' % path)
                continue
            if pfn:
            ...
            if os.path.isdir(path):
                ...
                if not len(fnames) and not len(subdirs):
                    print('Skipping %s because it is empty.' % dname)
                elif not len(fnames):
                    print('Skipping %s because it has no files in it. Subdirectories are not supported.' % dname)
